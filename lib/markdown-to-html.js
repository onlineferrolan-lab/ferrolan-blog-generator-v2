...
export function markdownToHtml(md) {
  ...
  let html = content
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    // Imágenes ![alt](src) — ANTES que los enlaces para que no se traguen el "!"
    .replace(/^!\[([^\]]*)\]\(([^)]+)\)$/gm, (_, alt, src) =>
      `<figure><img src="${src}" alt="${alt}" />${alt ? `<figcaption>${alt}</figcaption>` : ""}</figure>`
    )
  ...
  for (const line of lines) {
    ...
    if (!trimmed) continue;
    if (/^<(h[1-6]|ul|li|hr|p|\/p|\/ul|figure)/.test(trimmed)) {
      result.push(trimmed);
    } else {
